...
class UserController {
    ...
    getFriendsRequests = catchAsync(async (req, res, next) => {
        const id = req.params.id;
        const page = req.query.page || 1;
        const limit = req.query.limit || 10;
        const exists = await userService.checkUserExist(id);
        ...
        const friendsRequests = await userService.getFriendRequests(id, page, limit);
        //only get id firstName, lastName and picture
    ...
    removeFriend = catchAsync(async (req, res, next) => {
        const user = req.user;
    ...
    friendRequest = catchAsync(async (req, res, next) => {
        ...
        const friendToRequest = await User.findById(friendId);
